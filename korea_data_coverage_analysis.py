#!/usr/bin/env python3
"""
Korea Data Coverage Analysis
Analyzes all datasets to find the time periods where Korea data is available
"""
...
def detect_delimiter(file_path, sample_size=5):
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
        sample = file.read(1024 * sample_size)  # Read first few KB
...
def find_korea_columns_and_data(df):
    """Find columns that might contain Korea data and extract Korea rows"""
    # Extended Korea patterns including ISO codes and multilingual names
    korea_patterns = [
        # ISO codes
        'kor', 'kr', 'rok',
        # English
    ...
    # Check each column for Korea data
    for col in df.columns:
        col_lower = str(col).lower()
        
        # Check if this column might contain country information
        ...
        # Also check columns that might be in other languages
        ...
        if is_country_column or is_multilingual or col_lower in ['iso']:
            ...
            for idx, val in df[col].items():
                ...
                if any(pattern in val_str for pattern in korea_patterns):
                    ...
                    break  # Found Korea in this row, move to next row
    
    # Remove duplicates (same row found in multiple columns)
    unique_korea_data = []
    seen_rows = set()
...
def extract_year_info(df, korea_data):
    ...
    for korea_row in korea_data:
        ...
        # If no year found in year columns, check all columns for year-like values
        if not years:
...
def safe_read_csv(csv_file):
    """Safely read CSV file with automatic delimiter and encoding detection"""
    ...
    # If all fails, try with different delimiters
    for delimiter in [';', ',', '\t', '|']:
        for encoding in encodings:
            try:
                df = pd.read_csv(csv_file, 
...
def analyze_dataset_coverage():
    """Analyze time coverage for Korea data across all datasets"""
    ...
    # Analyze each dataset directory
    for dataset_dir in os.listdir(dataset_base):
        ...
        # Find all CSV files in this dataset
        csv_files = []
        ...
        for csv_file in csv_files:
            try:
                ...
                # Use safe CSV reading with automatic detection
                ...
                if korea_data:
                    ...
                    if years:
                        ...
                        print(f"    ⚠️  No clear year information found")
                        
                    # Show sample Korea values for debugging
    ...
    if len(all_dataset_years) >= 2:
        ...
        # Find years covered by at least N datasets
        for min_datasets in range(len(all_dataset_years), 0, -1):
            ...
            if common_years:
                ...
                if common_years:
                    ...
                    # Show which datasets contribute to the best period
